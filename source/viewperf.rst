...
Note that one would expect call 2514 to be glTexImage(level=9,
width=1,height=1) but it's not there.
...
brighter and match the reference image (however, AMD's rendering is much
...
2. The test never calls glClear(GL\_DEPTH\_BUFFER\_BIT) to initialize
   the depth buffer
